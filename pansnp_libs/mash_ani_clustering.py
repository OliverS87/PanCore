# Write the Rcscript required for mash ANI distance clustering into the
# output folder
from os import path
from os import remove

class MashAnoClusteringRscript:
    def __init__(self, out_path):
        self.out_path = out_path

    def write_script(self):
        with open(path.join(self.out_path, "mash_ani_clustering.r"), "w") as out_f:
            for line in self.get_script():
                out_f.write(line+"\n")

    def remove_script(self):
        remove(path.join(self.out_path, "mash_ani_clustering.r"))

    # sed -e 's/.*/"&",/' mash_ani_clustering.r
    def get_script(self):
        script = [
            "#!/usr/bin/env Rscript",
